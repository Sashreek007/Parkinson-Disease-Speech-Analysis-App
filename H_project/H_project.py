...
try:
    df = pd.read_csv('/Users/sashreek/Documents/H_project/parkinsons data.csv')
...
class State(rx.State):
    ...
    async def handle_upload(self, files: list[rx.UploadFile]):
        for file in files:
            ...
            try:
                features = extract_features(str(outfile))  # Extract 18 features
...
def index():
    return rx.vstack(
        rx.heading("Parkinson's Health Prediction"),
        rx.upload(
            rx.vstack(
                rx.button("Select Audio File", bg="white", border="1px solid rgb(107,99,246)"),
                rx.text("Drag and drop or click to select audio files"),
            ),
            id="audio_upload",
        ...
        rx.button("Process Audio", on_click=State.handle_upload(rx.upload_files(upload_id="audio_upload"))),
        rx.heading(State.prediction, margin_top="2em"),
        padding="2em",
    )
...
print(X_train.shape)  # Should print (number of samples, 18) for 18 features
